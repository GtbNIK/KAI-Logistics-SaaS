...
const QuickCreateZoneModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const result = await zoneService.createZone(formData);
            showSuccess('¡Éxito!', 'Zona creada correctamente');
            onSuccess({ value: result.id, label: `(${result.internalCode}) ${result.name}`, data: result });
            onClose();
        } catch (error) {
            console.error('Error creating zone:', error);
            showError('Error', error.response?.data?.message || 'No se pudo crear la zona');
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm px-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-400 uppercase ml-1">Nombre de la Zona</label>
                        <input
                            required
                            type="text"
                            value={formData.name}
