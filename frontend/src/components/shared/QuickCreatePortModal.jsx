...
import { X, Save, Loader2, Anchor } from 'lucide-react';
import portService from '../../services/port.service';
import { useToast } from '../../context/ToastContext';

const QuickCreatePortModal = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const result = await portService.createPort({ name, code });
            showSuccess('¡Éxito!', 'Puerto creado correctamente');
            onSuccess({ value: result.id, label: result.name, data: result });
            onClose();
        } catch (error) {
            console.error('Error creating port:', error);
            showError('Error', error.response?.data?.message || 'No se pudo crear el puerto');
        } finally {
            setLoading(false);
        }
    };
            ...
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-400 uppercase ml-1">Código del Puerto</label>
                        <input
                            required
                            type="text"
                            value={code}
                            onChange={e => setCode(e.target.value.toUpperCase())}
                            className="w-full px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all text-sm uppercase"
                            placeholder="Ej: COD-001"
                        />
                    </div>

                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-400 uppercase ml-1">Nombre del Puerto</label>
                        <input
                            required
                            type="text"
                            autoFocus
                            ...
                            placeholder="Ej: Puerto de La Guaira"
                        />
                    </div>
...
export default QuickCreatePortModal;
